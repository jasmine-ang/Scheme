...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        #self.bindings.get(symbol, 0)
        value = self.bindings.get(symbol, 'No value')
        if value == 'No value' and self.parent:
...
def do_define_macro(expressions, env):
    ...
    mac_name = target.first 
    formals = target.rest
    body = expressions.rest 

    env.define(mac_name, MacroProcedure(formals, body, env))
...
class Promise(object):
    ...
    def evaluate(self):
        if self.expression is not None:
            self.value = scheme_eval(self.expression, self.env)
